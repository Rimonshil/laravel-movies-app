...
<div class="movie-info border-b border-gray-900">
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row lg:flex-row">
        <div class="flex-none">
        <img src="{{ $popularActor['profile_path'] }}" alt="profile image" class="w-76">      
          <ul class="flex items-center mt-4">
            ...
            @if ($popularActor['homepage'])
                
            <li class="ml-6">
            <a href="{{ $popularActor['homepage'] }}" title="Website">
        ...
        <div class="ml-24">
        <h2 class="text-4xl font-semibold">{{ $popularActor['name'] }}</h2>
            ...
            <span class="ml-2">{{ $popularActor['birthday']}} ({{ $popularActor['age']}} years old) {{ $popularActor['place_of_birth'] }}</span>
            </div> 

            <p class="text-gray-300 mt-8">
                {{ $popularActor['biography'] }}
            </p>    

            <h4 class="font-semibold mt-12">Known for</h4>

            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
                ...
                <a href="{{route('movies.show', $movie['id'])}}" class="text-sm text-gray-400 hover:text-white mt-1" >{{ $movie['title']}}</a>
                </div>
...
<div class="movie-credits border-b border-gray-800">
    ...
    <div class="container mx-auto px-4 py-16">
        ...
        <li>{{ $item['release_year']}} &middot; <strong>{{ $item['title']}}</strong> {{ $item['character']}}</li>
            @endforeach
        </ul>
        </div>
